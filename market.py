...
class MarketAnalyzer:
    ...
    def load_market_data(self, symbols: List[str], start_date: str, end_date: str) -> Dict[str, pd.DataFrame]:
        """
        Load market data for given symbols and date range with enhanced error handling
        ...
        for symbol in symbols:
            try:
                # Enhanced yfinance data fetching with multiple attempts
                ...
                try:
                    ...
                    try:
                        data = yf.download(symbol, start=start_date, end=end_date, auto_adjust=True, prepost=True, progress=False)
                    except Exception as e2:
                        logger.warning(f"Method 2 failed for {symbol}: {e2}")
                        
                        # Method 3: Basic download without options
                        try:
                            data = yf.download(symbol, start=start_date, end=end_date, progress=False)
                        except Exception as e3:
                            logger.error(f"All methods failed for {symbol}: {e3}")
                            continue
                ...
                if data is not None and not data.empty:
                    ...
                    if all(col in data.columns for col in required_cols):
                        ...
                        # Remove any NaN values from the index
                        data = data.dropna()
